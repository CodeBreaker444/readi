...
export async function getToolList(
  ...
  const clientIds = [
    ...new Set(
      (data || [])
        .map((t) => t.tool_metadata?.fk_client_id)
...
export async function getModelList(ownerId: number) {
  const { data: ownerTools, error: toolError } = await supabase
    ...
    .eq('fk_owner_id', ownerId)
    .eq('tool_active', 'Y')
    .not('fk_model_id', 'is', null);

  if (toolError) throw toolError;

  const modelIds = [...new Set((ownerTools || []).map((t) => t.fk_model_id as number))];
  ...
  const { data, error } = await supabase
    .from('tool_model')
    .select('model_id, model_code, model_name, manufacturer')
...
export async function addModel(modelData: any) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      specifications:    modelData.technical_specs  || null,
      model_active:      'Y',
    })
    .select()
    .single();
...
export async function getMaintenanceDashboard(
  ...
  const maintenanceNeeded = filtered.filter((tool) => {
    ...
    return components.some((comp) => {
      ...
      const usagePct = (comp.current_usage_hours / comp.expected_lifespan_hours) * 100;
      return usagePct >= thresholdAlert;
    });
  });
  ...
  return {
    code: 1,
    message: 'Success',
    dataRows: maintenanceNeeded.length,

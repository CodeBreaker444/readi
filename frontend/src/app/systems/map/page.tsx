      ...
      <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={onlyDock}
                    onChange={(e) => setOnlyDock(e.target.checked)}
                    className="w-4 h-4 text-blue-600 border-gray-300 rounded"
                  />
                  <span className="text-sm text-gray-700">Dock only</span>
                </label>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-200 flex justify-between">
              <h2 className="font-medium text-gray-900">List</h2>
                  ...
                  {filteredTools.length === 0 ? (
                    ...
                    filteredTools.map(tool => (
                      <tr key={tool.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3">{tool.tool_code}</td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <button className="px-2 py-1 text-xs text-blue-600">Go</button>
                          <button className="px-2 py-1 text-xs text-gray-600">Details</button>
                        </td>
                      </tr>
                    ))
                  )}
          ...
          <div className="lg:col-span-2 bg-white rounded-lg shadow-sm">
            <div className="relative h-[420px] bg-gradient-to-br from-blue-50 to-indigo-50">
              <div className="absolute inset-0 flex items-center justify-center text-center">
                <div>
                  <div className="text-4xl mb-2">🗺️</div>
                  <p className="font-medium">Interactive Map</p>
                  <p className="text-sm text-gray-500">Leaflet map would render here</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div className="text-2xl font-semibold">{filteredTools.length}</div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="text-sm text-gray-500">Docks</div>
            <div className="text-2xl font-semibold text-blue-600">
              {filteredTools.filter(t => isDock(t)).length}
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="text-sm text-gray-500">Operational</div>
            <div className="text-2xl font-semibold text-green-600">
              {filteredTools.filter(t => t.tool_status === 'OPERATIONAL').length}
            </div>
          </div>
        </div>

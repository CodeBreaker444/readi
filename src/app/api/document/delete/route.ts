...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title: docTitle, docCode } = await deleteDocument({ document_id: parsed.data.document_id });

    logEvent({
      eventType: 'DELETE',
      entityType: 'document',
      entityId: parsed.data.document_id,
      description: `Deleted document '${docTitle ?? `#${parsed.data.document_id}`}'${docCode ? ` (code: ${docCode})` : ''}`,

import { getFlightLogWaypoints } from '@/backend/services/operation/flight-log-service';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getFlightLogWaypoints(missionId);
    return NextResponse.json({ code: 1, data: result });
  } catch (err) {
    console.error('[flight-logs/waypoints] GET error:', err);

import { updateAdminEmail } from '@/backend/services/company/owner-service';
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const { admin_user_id, new_email } = parsed.data;
        ...
        if (err?.message === 'A user with this email already exists') {
            return NextResponse.json({ code: 0, message: err.message }, { status: 409 });
        }

...
import MissionDetailSheet from '../../../components/client/MissionDetailSheet';
...
export default function ClientMissionsPage() {
  ...
  const columns = useMemo(
    () => getClientMissionColumns(t, isDark, setDetail),
    [t, isDark],
  );
      ...
      >
        <div className="mx-auto max-w-[1800px] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          ...
          <span className={cn('text-xs tabular-nums', textSub)}>
            {total} {total === 1
              ? t('clientPortal.missionCount_one', { count: total })
              : t('clientPortal.missionCount_other', { count: total })}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 px-4 sm:px-6 pb-8 space-y-4 max-w-[1800px] mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className={cn(
            ...
            <input
              className={cn('bg-transparent text-sm outline-none flex-1 min-w-0', isDark ? 'text-white' : 'text-slate-900')}
              ...
              <table className="w-full text-sm min-w-[700px]">
                ...
                <tbody>
                  {Array.from({ length: 8 }).map((_, row) => (
                    <tr key={row} className={cn('border-b last:border-0', isDark ? 'border-white/5' : 'border-slate-100')}>
                      {Array.from({ length: 8 }).map((_, col) => (
                        <td key={col} className="px-4 py-3">
                          <Skeleton className={cn('h-4', col === 0 ? 'w-24' : col === 2 ? 'w-16 rounded-full' : 'w-20')} />
                        </td>
                      ))}
                    </tr>
                  ))}
              ...
              <table className="w-full text-sm min-w-[700px]">
                <thead>
                  {table.getHeaderGroups().map((hg) => (
                    <tr key={hg.id} className={cn('border-b', isDark ? 'border-white/10 bg-white/[0.03]' : 'border-slate-200 bg-slate-50')}>
                      {hg.headers.map((header) => (
                        <th key={header.id} className={cn('px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider', textSub)}>

...
export default function CommunicationPage() {
  ...
  const fetchCommunications = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : t('organization.communication.toasts.loadFailed');
      toast.error(msg);
    } finally {
      setIsLoading(false);
    }
  ...
  function handleAddSuccess(newItem: Communication) {
    ...
    setIsAddOpen(false);
    toast.success(t('organization.communication.toasts.added'));
  }
  ...
  function handleEditSuccess(updatedItem: Communication) {
    ...
    handleEditClose();
    toast.success(t('organization.communication.toasts.updated'));
  }
  ...
  async function handleDelete(communicationId: number) {
    try {
      ...
      if (res.data.code !== 1) {
        toast.error(res.data.message ?? t('organization.communication.toasts.deleteFailed'));
        return;
      }
      ...
      toast.success(t('organization.communication.toasts.deleted'));
    } catch (err: any) {
          ...
          <div className="mx-auto max-w-[1800px] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`font-semibold text-base tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>
                  {t('organization.communication.title')}
                </h1>
                <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                  {t('organization.communication.subtitle')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {t('organization.communication.refresh')}
              </Button>
              ...
              >
                ...
                {t('organization.communication.newCommunication')}
              </Button>
            </div>
          </div>
        </div>

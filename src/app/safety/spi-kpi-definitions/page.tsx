...
const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('safety.spiKpi.toast.loadError'))  
    } finally {
      setIsLoading(false)
    }
  }, [appliedFilters, t])
...
const handleSave = async (payload: any) => {
    ...
    try {
      ...
      if (response.data.code === 1 && savedRecord) {
        ...
        setDialogOpen(false)
        toast.success(t('safety.spiKpi.toast.recordSynced')) 
      } else {
      ...
      const message = axios.isAxiosError(err)
        ...
        : t('safety.spiKpi.toast.saveFailed') 
      toast.error(message)
    } finally {
      setIsSaving(false)
    }
  ...
  const handleToggle = async (row: SpiKpiDefinition) => {
    ...
    try {
      ...
      if (res.data.code === 1) {
       ...
       toast.success(
          activeStatus === 1 
            ? t('safety.spiKpi.toast.activated') 
            : t('safety.spiKpi.toast.deactivated')
        )
      }
    } catch (err) {
      toast.error(t('safety.spiKpi.toast.toggleFailed'));
    } finally {
      setIsToggling(null)
    }
  }
  ...
  const handleLogValue = async (payload: {
    ...
    try {
      ...
      if (response.data.code === 1) {
        ...
        toast.success(t('safety.spiKpi.toast.measurementLogged'))
      } else {
        throw new Error(response.data.error || 'Server returned an error')
      }
    } catch (err) {
      const message = axios.isAxiosError(err) ? err.response?.data?.error : t('safety.spiKpi.toast.logFailed')
      toast.error(message)
    } finally {
      setIsLogging(false)
    }
  ...
  const columns = useMemo(
    () => getIndicatorColumns(isDark, openEdit, handleToggle, isToggling, AREA_COLORS, t, openLogValue),
    [isDark, isToggling]
  )
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-slate-900 text-white' : 'bg-slate-50 text-gray-900'
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className={`font-semibold text-base tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>
                {t('safety.spiKpi.pageTitle')}
              </h1>
              <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                {t('safety.spiKpi.pageSubtitle')}
              </p>
            </div>
          </div>
      ...
      <div className="mx-auto px-6 py-8 space-y-6 animate-slide-up">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: t('safety.spiKpi.stats.totalIndicators'), value: data.length, icon: Activity, color: 'text-blue-500' },
            { label: t('safety.spiKpi.stats.activeMonitors'), value: data.filter(d => d.is_active === 1).length, icon: TrendingUp, color: 'text-emerald-500' },
            { label: t('safety.spiKpi.stats.operationsArea'), value: data.filter(d => d.indicator_area === 'OPERATIONS').length, icon: ShieldCheck, color: 'text-amber-500' },
          ].map((stat) => (
        ...
        <div className={`flex items-center gap-2.5 px-4 py-2 rounded-lg border text-xs ${isDark ? 'bg-violet-500/10 border-violet-500/20 text-violet-300' : 'bg-violet-50 border-violet-200 text-violet-700'}`}>
          ...
          {t('safety.spiKpi.infoBanner')} <strong>{t('safety.spiKpi.infoBannerLogValue')}</strong> {t('safety.spiKpi.infoBannerSuffix')}
        </div>
        ...
        <div className={`p-4 rounded-xl border transition-all ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200 shadow-sm'
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[280px]">
              <label className={`text-[10px] uppercase tracking-widest font-bold mb-1.5 block ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{t('safety.spiKpi.filters.searchDefinitions')}</label>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                <Input
                  value={filters.q}
                  onChange={(e) => setFilters(f => ({ ...f, q: e.target.value }))}
                  placeholder={t('safety.spiKpi.filters.searchPlaceholder')}
            ...
            {[
              { label: t('safety.spiKpi.filters.area'), key: 'area' as const, opts: [['ALL', t('safety.spiKpi.filters.allAreas')], ['COMPLIANCE', t('safety.spiKpi.areas.COMPLIANCE')], ['TRAINING', t('safety.spiKpi.areas.TRAINING')], ['OPERATIONS', t('safety.spiKpi.areas.OPERATIONS')], ['MAINTENANCE', t('safety.spiKpi.areas.MAINTENANCE')]] },
              { label: t('safety.spiKpi.filters.status'), key: 'active' as const, opts: [['ALL', t('safety.spiKpi.filters.allStatus')], ['1', t('safety.spiKpi.filters.activeOnly')], ['0', t('safety.spiKpi.filters.inactiveOnly')]] }
            ].map((fConfig) => (
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={columns.length} className="h-64 text-center">
                    ...
                    <p className="text-slate-500 text-sm">{t('safety.spiKpi.table.noResults')}</p>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>

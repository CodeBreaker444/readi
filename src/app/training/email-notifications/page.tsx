...
import { TRAINING_EVENTS } from "@/config/types/email-notification";
...
export default function TrainingEmailNotificationsPage() {
    ...
    getOwnerData().then((data) => {
      if (data) {
        setOwnerData({
          ...
          emailEnabled: data.trainingEmailEnabled || false,
        });
      }
      setLoading(false);
    });
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                  {t('settings.emailNotifications.training.title')}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className={`min-h-screen ${isDark ? "bg-slate-900" : "bg-slate-50"}`}>
        ...
        <div className="mx-auto px-4 sm:px-6 py-6 sm:py-8">
          <div className={`flex items-center justify-center p-8 rounded-xl border text-center ${isDark ? "bg-slate-800 border-slate-700 text-slate-400" : "bg-white border-slate-200 text-slate-500"}`}>
            <p>{t('settings.emailNotifications.training.unableToLoad')}</p>
          </div>
        </div>
      </div>
    );
      ...
      <main className="mx-auto px-4 sm:px-6 py-6">
          ...
          >
            <div className="flex-1 min-w-0">
              <h3 className={`font-medium text-sm ${isDark ? "text-white" : "text-slate-900"}`}>
                {ownerData.emailEnabled ? t('settings.emailNotifications.training.enabled') : t('settings.emailNotifications.training.disabled')}
              </h3>
              <p className={`text-xs mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                {ownerData.emailEnabled
                  ? t('settings.emailNotifications.training.enabledDescription')
                  : t('settings.emailNotifications.training.disabledDescription')}
              </p>
            </div>
            <span
            ...
            >
              {ownerData.emailEnabled ? t('settings.emailNotifications.training.enabledLabel') : t('settings.emailNotifications.training.disabledLabel')}
            </span>
          </div>
        </div>
        ...
        <div className="w-full">
          <ModuleEmailNotificationConfig
            ...
            translationPrefix="settings.emailNotifications.training"
          />
        </div>
      </main>
    </div>

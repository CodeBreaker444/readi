...
export default function DashboardClient({ ownerId, userProfileCode, userId, initialData }: DashboardClientProps) {
  ...
  const cardTitle  = cn('text-sm font-semibold', isDark ? 'text-white' : 'text-gray-800');
  const eyebrow    = cn('text-xs font-semibold uppercase tracking-widest mb-0.5', isDark ? 'text-slate-500' : 'text-gray-400');
        ...
        {stats.map((s, i) => {
          ...
          return (
            <div key={i} className={cn(card, 'relative overflow-hidden p-5 flex flex-col gap-3')}>
              <div className="absolute left-0 top-0 h-full w-0.5 rounded-l-xl" style={{ backgroundColor: s.accent }} />
              ...
              {(data?.readi_mission_result_chart?.labels ?? ['Completed', 'In Progress', 'Failed'])
                .map((label: string, i: number) => {
                  const val = data?.readi_mission_result_chart
                    ...
                    : ['65%', '25%', '10%'][i];
                  const total = data?.readi_mission_result_chart
                    ? data.readi_mission_result_chart.series.reduce((a: number, b: number) => a + b, 0)
                    : 100;
                  ...
                  return (
                    <div key={label}>
                      ...
                      <div className={cn('h-1 w-full rounded-full overflow-hidden', isDark ? 'bg-slate-700' : 'bg-gray-100')}>

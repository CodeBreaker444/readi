      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="space-y-1.5">
          <Label className={`text-[10px] font-bold uppercase tracking-widest ${labelClass}`}>
            {t('missionPlanning.filter.client')}
          ...
          <Select value={clientId} onValueChange={setClientId}>
            <SelectTrigger className={`h-9 text-sm focus:ring-blue-500/20 ${inputBgClass}`}>
              <SelectValue placeholder={t('missionPlanning.filter.allClients')} />
            </SelectTrigger>
            <SelectContent className={isDark ? "bg-slate-800 border-slate-700 text-white" : ""}>
              <SelectItem value="0">{t('missionPlanning.filter.allClients')}</SelectItem>
              {clients.map((c) => (
        ...
        <div className="space-y-1.5">
          ...
          <Select value={pilotId} onValueChange={setPilotId}>
            <SelectTrigger className={`h-9 text-sm focus:ring-blue-500/20 ${inputBgClass}`}>
              <SelectValue placeholder={t('missionPlanning.filter.allPilots')} />
            </SelectTrigger>
            <SelectContent className={isDark ? "bg-slate-800 border-slate-700 text-white" : ""}>
              <SelectItem value="0">{t('missionPlanning.filter.allPilots')}</SelectItem>
              {pilots.map((p) => (
        ...
        <div className="space-y-1.5">
          <Label className={`text-[10px] font-bold uppercase tracking-widest ${labelClass}`}>
            {t('missionPlanning.filter.evaluation')}
          </Label>
          <Select value={evaluationId} onValueChange={setEvaluationId}>
            <SelectTrigger className={`h-9 text-sm focus:ring-blue-500/20 ${inputBgClass}`}>
              <SelectValue placeholder={t('missionPlanning.filter.allEvaluations')} />
            </SelectTrigger>
            <SelectContent className={isDark ? "bg-slate-800 border-slate-700 text-white" : ""}>
              <SelectItem value="0">{t('missionPlanning.filter.allEvaluations')}</SelectItem>
              {evaluations.map((e) => (
        ...
        <div className="space-y-1.5">
          <Label className={`text-[10px] font-bold uppercase tracking-widest ${labelClass}`}>
            {t('missionPlanning.filter.planning')}
          </Label>
          <Select value={planningId} onValueChange={setPlanningId}>
            <SelectTrigger className={`h-9 text-sm focus:ring-blue-500/20 ${inputBgClass}`}>
              <SelectValue placeholder={t('missionPlanning.filter.allPlannings')} />
            </SelectTrigger>
            <SelectContent className={isDark ? "bg-slate-800 border-slate-700 text-white" : ""}>
              <SelectItem value="0">{t('missionPlanning.filter.allPlannings')}</SelectItem>
              {plannings.map((p) => (
        ...
        <div className="space-y-1.5">
          <Label className={`text-[10px] font-bold uppercase tracking-widest ${labelClass}`}>{t('missionPlanning.filter.updateFrom')}</Label>
          <Input
            type="date"
            value={dateStart}
        ...
        <div className="space-y-1.5">
          <Label className={`text-[10px] font-bold uppercase tracking-widest ${labelClass}`}>{t('missionPlanning.filter.updateTo')}</Label>
          <Input
            type="date"
            value={dateEnd}
        ...
        <div className="flex items-end gap-2 sm:col-span-2">
          ...
          >
            <Search className="h-3.5 w-3.5" /> {t('missionPlanning.filter.search')}
          </Button>
          <Button
            variant="outline"
            onClick={handleReset}
          ...
          >
            <RotateCcw className="h-3.5 w-3.5" /> {t('missionPlanning.filter.reset')}
          </Button>
        </div>
      </div>
    </div>

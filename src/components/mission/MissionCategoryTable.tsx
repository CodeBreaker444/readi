        ...
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className={`border-b ${isDark ? 'border-white/[0.06] hover:bg-transparent' : 'border-gray-100 hover:bg-transparent'}`}>
                {headerGroup.headers.map((header) => (
                  <TableHead key={header.id} className={`h-10 px-5 text-[10px] font-semibold uppercase tracking-[0.08em] ${isDark ? 'text-gray-500 bg-white/[0.02]' : 'text-gray-400 bg-gray-50/60'}`}>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className={`group transition-colors border-b ${isDark ? 'border-white/[0.04] hover:bg-white/[0.03]' : 'border-gray-50 hover:bg-gray-50/50'}`}>

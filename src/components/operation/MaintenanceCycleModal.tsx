...
export function MaintenanceCycleModal({
  ...
  const formatLastMaintenance = (dateStr: string | null): string => {
    if (!dateStr) return t("planning.files.noFiles");
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) (onSkip ?? onClose)(); }}>
      ...
      >
          ...
          {systemData && (
            <div className="mt-3 mb-3">
              ...
              >
                ...
                />
                {t("operations.table.detail.droneSystem")} {STATUS_CONFIG[systemData.system_status].label}
              </Badge>
            </div>
          )}
        </DialogHeader>
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn("text-[11px] font-medium", isDark ? "text-slate-300" : "text-slate-700")}>
                          {comp.component_code}
                        </span>
                        {comp.serial_number && (
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-3">
                      ...
                      <CycleProgressBar
                        ...
                        label={t("operations.table.detail.duration")}
                        icon={Clock}
                        status={comp.status}
                        isDark={isDark}
                        isHours
                    ...
                    {comp.status === "DUE" && (
                      <div className={cn("mt-3 flex items-center gap-2 rounded-lg px-3 py-2 text-[11px]", isDark ? "bg-rose-500/10 text-rose-400" : "bg-rose-50 text-rose-600")}>
                        ...
                        <span>{t("operations.table.detail.maintenance")} {t("operations.table.status.aborted")}</span>
                      </div>
                    )}
                  </div>
                );
        ...
        <div className={cn("flex items-center justify-between gap-2 px-6 py-4 border-t", isDark ? "border-white/[0.06] bg-slate-900/30" : "border-slate-200 bg-slate-50")}>
          {onSkip ? (
            <div className={cn("flex items-center gap-1.5 rounded-lg px-3 py-2 text-[11px]", isDark ? "bg-amber-500/10 text-amber-400" : "bg-amber-50 text-amber-600")}>
              <AlertTriangle className="h-3.5 w-3.5 shrink-0" />
              <span>{t("operations.board.toast.revertDesc")}</span>
            </div>
          ) : <div />}
          <div className="flex items-center gap-2">
            <Button

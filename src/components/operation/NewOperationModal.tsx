...
export function NewOperationModal({ open, onClose, onSuccess, isDark, editOperation, onSaved, logSerialNumber, createPrefill }: NewOperationModalProps) {
    ...
    const canNext = () => {
        ...
        if (step === 2) {
            ...
            if (logSerialNumber) {
                ...
                if (!serialInList(selected?.drone_serial_numbers, logSerialNumber)) return false
            }
            if (opType === 'PDRA') {
    ...
    // D-Flight authorization only ever runs for PDRA missions — OPEN/STS-01/STS-02
    // never submit trajectory data, so the picker has nothing to gate for them.
    const uspaceRequired = dFlightEnabled && opType === 'PDRA'
                    ...
                    <div className={cn('flex items-center justify-between px-6 pb-6 pt-2 border-t', isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-muted/20')}>
                        ...
                        {step < 4 ? (
                            ...
                            <Button
                                ...
                                disabled={isSubmitting || !pilotId || !schedulerForm.missionCode.trim() || !schedulerForm.lucId || !schedulerForm.typeId || !schedulerForm.categoryId || !schedulerForm.scheduledStart || (uspaceRequired && !uspaceId)}

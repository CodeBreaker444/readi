...
export function EvaluationTable({ onView, isDark }: { onView?: (ev: Evaluation) => void; isDark?: boolean }) {
  ...
  const fetchEvaluations = async () => {
    try {
      ...
      toast.error(t('planning.evaluation.loadEvaluationsError'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success(t('planning.evaluation.deleteSuccess'));
      setDeleteTarget(null);
      fetchEvaluations();
    } catch (err) {
      toast.error(t('planning.evaluation.deleteFailed'));
    } finally {
      setIsDeleting(false);
    }
  }
    ...
    <>
      <div className="flex items-center justify-between gap-3 mb-3">
        <div className="relative max-w-sm flex-1">
          ...
          <Input
            placeholder={t('planning.evaluation.search')}
            value={globalFilter}
        ...
        <div className={cn("text-xs font-medium", isDark ? "text-slate-500" : "text-slate-400")}>
          {table.getFilteredRowModel().rows.length} {t('planning.evaluation.recordsFound')}
        </div>
      </div>
                ...
                <TableCell colSpan={columns.length} className="h-[300px] text-center">
                  <div className="flex flex-col items-center justify-center space-y-3 animate-in fade-in zoom-in duration-300">
                    ...
                    <div className="space-y-1">
                      <p className={cn("text-sm font-medium", isDark ? "text-slate-300" : "text-slate-600")}>{t('planning.evaluation.noEvaluations')}</p>
                      <p className={cn("text-xs", isDark ? "text-slate-500" : "text-slate-400")}>
                        {globalFilter ? t('planning.evaluation.adjustFilters') : t('planning.evaluation.createNew')}
                      </p>
                    </div>
                  </div>
                </TableCell>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent className={isDark ? "bg-slate-900 border-slate-800" : ""}>
          <AlertDialogHeader>
            <AlertDialogTitle className={isDark ? "text-slate-100" : ""}>{t('planning.evaluation.deleteTitle')}</AlertDialogTitle>
            <AlertDialogDescription className={isDark ? "text-slate-400" : ""}>
              {t('planning.evaluation.deleteDescPrefix')} <strong className={isDark ? "text-slate-200" : ""}>EVAL_{deleteTarget?.evaluation_id}</strong>?
              {t('planning.evaluation.deleteDescSuffix')}

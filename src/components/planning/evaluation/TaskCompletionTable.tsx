...
import { CheckCircle2, ListChecks, MessageSquarePlus } from 'lucide-react';
...
export function TaskCompletionTable({ evaluationId, ownerId, onAllCompleted }: Props) {
  ...
  function handleOpenAction(task: EvaluationTask) {
    if (task.task_type === 'checklist') {
      setModal({ type: 'checklist', task });
    } else if (task.task_type === 'assignment') {
      setModal({ type: 'assignment', task });
    } else if (task.task_type === 'communication') {
      setModal({ type: 'communication', task });
    }
  }
  ...
  async function handleCommunicationSent(task?: EvaluationTask) {
    if (task?.task_id) {
      try {
        ...
        // Non-fatal — communication was still sent
      }
    }
    fetchTasks();
    closeModal();

...
export default function DuplicateSystemModal({
    ...
    const loadDraft = async () => {
      ...
      try {
        ...
        if (systemJson.code !== 1) {
          toast.error(systemJson.message || t('systems.components.duplicateSystem.toasts.loadSourceFailed'));
          return;
        }
        if (componentJson.code !== 1) {
          toast.error(componentJson.message || t('systems.components.duplicateSystem.toasts.loadComponentsFailed'));
          return;
        }

        const sourceSystem = (systemJson.data || []).find((s: any) => s.tool_id === sourceSystemId);
        if (!sourceSystem) {
          toast.error(t('systems.components.duplicateSystem.toasts.sourceNotFound'));
          return;
        }
        ...
        toast.error(t('systems.components.duplicateSystem.toasts.prepareDraftError'));
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t('systems.components.duplicateSystem.toasts.duplicateSuccess'));
      onSuccess();
    } catch (error: any) {
      toast.error(error?.message || t('systems.components.duplicateSystem.toasts.duplicateFailed'));
    } finally {
      setSaving(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`w-[95vw] max-w-6xl h-[90vh] flex flex-col overflow-hidden p-0 gap-0 ${isDark ? 'bg-slate-800 border-slate-700' : ''}`}>
        <DialogHeader className="px-6 pt-5 pb-4 shrink-0 border-b border-slate-200 dark:border-slate-700">
          <DialogTitle className={isDark ? 'text-white' : ''}>{t('systems.components.duplicateSystem.title')}</DialogTitle>
        </DialogHeader>
        ...
        {loadingDraft || !systemDraft ? (
          ...
          <Tabs defaultValue="system" className="flex flex-col flex-1 overflow-hidden">
            <TabsList className="shrink-0 mx-6 mt-4 mb-0 w-fit">
              <TabsTrigger value="system">{t('systems.components.duplicateSystem.tabs.systemDetails')}</TabsTrigger>
              <TabsTrigger value="components">{t('systems.components.duplicateSystem.tabs.components', { count: componentsDraft.length })}</TabsTrigger>
            </TabsList>

            <TabsContent value="system" className="flex-1 overflow-y-auto px-6 py-4 mt-0 data-[state=inactive]:hidden">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.code')}</Label>
                  <Input className={inputCls} value={systemDraft.tool_code} onChange={(e) => handleSystemChange('tool_code', e.target.value)} />
                </div>
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.description')}</Label>
                  <Input className={inputCls} value={systemDraft.tool_desc} onChange={(e) => handleSystemChange('tool_desc', e.target.value)} />
                </div>
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.latitude')}</Label>
                  <Input className={inputCls} value={systemDraft.tool_latitude} onChange={(e) => handleSystemChange('tool_latitude', e.target.value)} />
                </div>
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.longitude')}</Label>
                  <Input className={inputCls} value={systemDraft.tool_longitude} onChange={(e) => handleSystemChange('tool_longitude', e.target.value)} />
                </div>
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.status')}</Label>
                ...
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.activationDate')}</Label>
                ...
                <div>
                  <Label className="pb-2">{t('systems.components.duplicateSystem.systemFields.client')}</Label>
            ...
            <TabsContent value="components" className="flex-1 overflow-y-auto px-6 py-4 mt-0 space-y-3 data-[state=inactive]:hidden">
              {componentsDraft.length === 0 ? (
                <p className="text-sm text-slate-400">{t('systems.components.duplicateSystem.noComponents')}</p>
              ) : (
                  ...
                  >
                    <CollapsibleTrigger className="w-full p-3 flex items-center justify-between text-left">
                      <div>
                        <p className={`text-sm font-semibold ${isDark ? 'text-slate-100' : 'text-slate-800'}`}>
                          {component.component_code || component.component_name || t('systems.components.duplicateSystem.componentIndex', { index: index + 1 })}
                        </p>
                        <p className="text-xs text-slate-400">{component.component_type || t('systems.components.duplicateSystem.noType')}</p>
                      </div>
                    ...
                    <CollapsibleContent className="p-3 pt-0">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.type')}</Label>
                          <Input className={inputCls} value={component.component_type} onChange={(e) => handleComponentChange(component.tempId, 'component_type', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.code')}</Label>
                          <Input className={inputCls} value={component.component_code} onChange={(e) => handleComponentChange(component.tempId, 'component_code', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.name')}</Label>
                          <Input className={inputCls} value={component.component_name} onChange={(e) => handleComponentChange(component.tempId, 'component_name', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.serialNumber')}</Label>
                          <Input className={inputCls} value={component.component_sn} onChange={(e) => handleComponentChange(component.tempId, 'component_sn', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.description')}</Label>
                          <Input className={inputCls} value={component.component_desc} onChange={(e) => handleComponentChange(component.tempId, 'component_desc', e.target.value)} />
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <Select value={component.component_status} onValueChange={(v) => handleComponentChange(component.tempId, 'component_status', v)}>
                            <SelectTrigger className={inputCls}><SelectValue /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value="OPERATIONAL">{t('systems.components.common.statusOptions.operational')}</SelectItem>
                              <SelectItem value="NOT_OPERATIONAL">{t('systems.components.common.statusOptions.notOperational')}</SelectItem>
                        ...
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.parentComponent')}</Label>
                          <Select value={component.fk_parent_component_id || '_none'} onValueChange={(v) => handleComponentChange(component.tempId, 'fk_parent_component_id', v)}>
                            <SelectTrigger className={inputCls}><SelectValue /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value="_none">{t('systems.components.duplicateSystem.placeholders.noneTopLevel')}</SelectItem>
                              {(parentOptionsBySourceId[component.sourceId] || []).map((opt) => (
                        ...
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.activationDate')}</Label>
                          <Input type="date" className={inputCls} value={component.component_activation_date} onChange={(e) => handleComponentChange(component.tempId, 'component_activation_date', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.purchaseDate')}</Label>
                          <Input type="date" className={inputCls} value={component.component_purchase_date} onChange={(e) => handleComponentChange(component.tempId, 'component_purchase_date', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.vendor')}</Label>
                          <Input className={inputCls} value={component.component_vendor} onChange={(e) => handleComponentChange(component.tempId, 'component_vendor', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.guarantee')}</Label>
                          <Input type="number" className={inputCls} value={component.component_guarantee_day} onChange={(e) => handleComponentChange(component.tempId, 'component_guarantee_day', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.maintenanceCycle')}</Label>
                          <Input className={inputCls} value={component.maintenance_cycle} onChange={(e) => handleComponentChange(component.tempId, 'maintenance_cycle', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.cycleHours')}</Label>
                          <Input type="number" className={inputCls} value={component.maintenance_cycle_hour} onChange={(e) => handleComponentChange(component.tempId, 'maintenance_cycle_hour', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.cycleDays')}</Label>
                          <Input type="number" className={inputCls} value={component.maintenance_cycle_day} onChange={(e) => handleComponentChange(component.tempId, 'maintenance_cycle_day', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.cycleFlights')}</Label>
                          <Input type="number" className={inputCls} value={component.maintenance_cycle_flight} onChange={(e) => handleComponentChange(component.tempId, 'maintenance_cycle_flight', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.batteryRatio')}</Label>
                          <Input type="number" step="0.01" className={inputCls} value={component.battery_cycle_ratio} onChange={(e) => handleComponentChange(component.tempId, 'battery_cycle_ratio', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.c2Platform')}</Label>
                          <Input className={inputCls} value={component.cc_platform} onChange={(e) => handleComponentChange(component.tempId, 'cc_platform', e.target.value)} />
                        </div>
                        <div>
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.gcs')}</Label>
                          <Input className={inputCls} value={component.gcs_type} onChange={(e) => handleComponentChange(component.tempId, 'gcs_type', e.target.value)} />
                        </div>
                        <div className="md:col-span-2">
                          <Label className="pb-2">{t('systems.components.duplicateSystem.componentFields.dccDroneId')}</Label>
                          <Input className={inputCls} value={component.dcc_drone_id} onChange={(e) => handleComponentChange(component.tempId, 'dcc_drone_id', e.target.value)} />
                        </div>
                      </div>
                    </CollapsibleContent>
                  </Collapsible>
        ...
        <div className="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-slate-200 dark:border-slate-700">
          <Button type="button" variant="outline" onClick={onClose}>{t('systems.components.duplicateSystem.buttons.cancel')}</Button>
          <Button type="button" disabled={saving || loadingDraft || !systemDraft} onClick={handleSave} className="bg-violet-600 hover:bg-violet-700 text-white">
            ...
            {saving ? t('systems.components.duplicateSystem.buttons.savingCopy') : t('systems.components.duplicateSystem.buttons.saveDuplicate')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>

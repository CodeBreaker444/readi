    ...
    <div className={`flex flex-wrap gap-x-6 gap-y-2 text-xs items-center ${textCls}`}>
      ...
      <span className="flex items-center gap-1.5">
        <svg width="18" height="18" viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg">
          <rect x="6.5" y="11.5" width="15" height="9" rx="1.6" fill="#1976d2" fillOpacity={isDark ? "0.4" : "0.25"} stroke="#1976d2" strokeWidth="2"/>
          <path d="M5.5 12 L14 6 L22.5 12" fill="#1976d2" fillOpacity={isDark ? "0.3" : "0.15"} stroke="#1976d2" strokeWidth="2" strokeLinejoin="round"/>
      ...
      <span className="flex items-center gap-1.5">
        <svg width="18" height="18" viewBox="0 0 26 26" xmlns="http://www.w3.org/2000/svg">
          <circle cx="13" cy="12" r="7.5" fill="#2e7d32" fillOpacity={isDark ? "0.35" : "0.20"} stroke="#2e7d32" strokeWidth="2"/>
      ...
      {statusItems.map((i) => (
        <span key={i.label} className="flex items-center gap-1.5">
          <span className="inline-block w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: i.color }} />
          {i.label}
        </span>
      ))}
    </div>

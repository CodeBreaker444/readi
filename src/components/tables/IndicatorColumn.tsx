...
export const getIndicatorColumns = (
  ...
  onLogValue?: (row: SpiKpiDefinition) => void,
    ...
    columnHelper.accessor("indicator_code", {
      header: t('safety.spiKpi.table.indicatorCode'),
      cell: (info) => (
    ...
    columnHelper.accessor("indicator_area", {
      header: t('safety.spiKpi.table.area'),
      cell: (info) => (
        <Badge
          variant="outline"
    ...
    columnHelper.display({
      id: "details",
      header: t('safety.spiKpi.table.name'),
      cell: ({ row }) => (
        <div>
   ...
   header: t('safety.spiKpi.table.target'),
    ...
    header: t('safety.spiKpi.table.status'),
      cell: (info) => {
        ...
        return (
          <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${active
              ? "bg-green-50 text-green-700 border-green-200"
              : "bg-gray-50 text-gray-600 border-gray-200"
            }`}>
            <span className={`w-1.5 h-1.5 rounded-full ${active ? "bg-green-600 animate-pulse" : "bg-gray-400"}`} />
        {active ? t('safety.spiKpi.table.active') : t('safety.spiKpi.table.disabled')}
          </div>
        );
      },
    }),
        ...
        <div className="flex items-center justify-end gap-2">
          {onLogValue && (
            <Button
              ...
              title={t('safety.spiKpi.table.logMeasurement')}
